...
export default function StatusBar() {
  ...
  return (
    <footer className="pointer-events-none absolute bottom-3 left-3 flex items-center gap-2">
      <span className="rounded-full bg-black/60 px-2.5 py-1 text-[11px] text-white/80 backdrop-blur">
        ● {STATUS_LABEL[saveStatus] ?? saveStatus}
      </span>
      <span className="max-w-[50vw] truncate rounded-full bg-black/60 px-2.5 py-1 text-[11px] text-white/60 backdrop-blur">
        {hint}
      </span>
    </footer>
  )
