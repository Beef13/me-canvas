...
- 2026-09-16: No `backdrop-blur` over the canvas — it forces a full-screen filter recompute every repaint and tanks drag FPS in WKWebView. Solid translucent backgrounds only.
- 2026-09-16: Collab = PeerJS star topology, no backend. tldraw store gives everything: `listen` entries carry `source` (skip non-user for echo suppression), `mergeRemoteChanges` marks remote (undo-safe), `getStoreSnapshot`/`loadStoreSnapshot` are store METHODS. Blob-URL assets must travel as bytes (structured-cloneable over data channels).
